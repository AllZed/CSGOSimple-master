...
{
	static auto inReload = *(uint32_t*)(Utils::PatternScan("client.dll", "C6 87 ? ? ? ? ? 8B 06 8B CE FF 90") + 2);
...
int C_BasePlayer::GetSequenceActivity(int sequence)
{
	...
	static auto get_sequence_activity = reinterpret_cast<int(__fastcall*)(void*, studiohdr_t*, int)>(Utils::PatternScan("client.dll", "55 8B EC 53 8B 5D 08 56 8B F1 83"));
...
bool C_BasePlayer::HasC4()
{
	static auto fnHasC4
		= reinterpret_cast<bool(__thiscall*)(void*)>(
			Utils::PatternScan("client.dll", "56 8B F1 85 F6 74 31")
			);
...
{
	static DWORD addr = (DWORD)Utils::PatternScan("client.dll", "80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81");
